...
} from "@/store/api/subsidy-contribution.api"
import {
  useStrataQuery,
...
// Types
import { SubsidyContribution } from "@/types/SubsidyContribution"
...
export default function SubsidyContributionPage() {
  ...
  const { data, refetch, isLoading } = useSubsidyContributionsQuery({ search: "", page: currentPage, limit: 10 });
  const [storeTariff, storeTariffResult] = useStoreSubsidyContributionsMutation();
  const [updateTariff, updateTariffResult] = useUpdateSubsidyContributionsMutation();
  const [toggleTariff, toggleTariffResult] = useToggleSubsidyContributionsMutation();
    ...
    <div className="px-4 sm:px-6 lg:px-8">
      <div className="sm:flex sm:items-center">
        <div className="sm:flex-auto">
          <h1 className="text-base font-semibold text-gray-900">Subsidios y Contribuciones</h1>
          <p className="mt-2 text-sm text-gray-700">
            Listado de todos los subsidios y contribuciones, incluyendo su tipo y valor.
          </p>
        </div>
          ...
          <>
            <table className="min-w-full divide-y divide-gray-300">
              ...
              <tbody className="divide-y divide-gray-200 bg-white">
                { data && data?.data.map((user: SubsidyContribution, index: number) => (
                  <tr key={index+1}>
                    ...
                    <td className="hidden px-3 py-4 text-sm whitespace-nowrap text-gray-500 sm:table-cell">
                      {user.value.toLocaleString('es-CO', { style: 'currency', currency: 'COP' })}
                    </td>
